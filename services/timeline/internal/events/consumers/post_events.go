...
type PostEventsConsumer struct {
	log               *zap.Logger
	postEventsHandler *handlers.PostEventsHandler
	ch                *amqp091.Channel
	workerCount       int
	wg                sync.WaitGroup
}

func NewPostEventsConsumer(
	log *zap.Logger,
	ch *amqp091.Channel,
	db *gorm.DB,
	workerCount int,
) *PostEventsConsumer {
	consumerLog := log.With(zap.String("consumer_name", "timeline_post_events"))
	postEventsHandler := handlers.NewPostEventsHandler(consumerLog, db)
...
func (f *PostEventsConsumer) Start(ctx context.Context) error {
	...
	if err = f.ch.QueueBind(
		q.Name,
		events.AllPostEvents, // captures user.created and user.updated
		constants.SocmedExchangeName,
		false,
		nil,
	); err != nil {
	...
	msgs, err := f.ch.Consume(config.PostEventsQueue, "", false, false, false, false, nil)
	if err != nil {
		...
		go func(workerID int) {
			...
			for {
				...
				switch msg.RoutingKey {
					case events.PostCreated, events.PostUpdated:
						f.postEventsHandler.HandlePostCreatedOrUpdatedEvent(msg)
					case events.PostDeleted:
						f.postEventsHandler.HandlePostDeletedEvent(msg)
					}

					workerLog.Info("Post event handled successfully", extraLogFields...)
				}
			}
		}(workerID)
	}
...
func (f *PostEventsConsumer) Wait(ctx context.Context) {
